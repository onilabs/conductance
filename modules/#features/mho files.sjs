...
@desc
  ...
  When you run `conductance serve` with no arguments, `default.mho` is
  loaded from the current directory if it exists. Otherwise, the default
  server configuration included in conductance is used.

  To run the server with a file other than `./default.config`, pass it

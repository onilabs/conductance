...
exports.main = function(client, serverId, singleton) {
	...
	function runServer() {
		...
		waitfor {
			...
			handleOps(client, serverId, info);
		} and {
...
function appRunLoop(client, serverId, info, app, id, withLoad, alreadyRunning) {
	...
	withLoad {||
		try {
			...
			waitfor {
				var op_endpoint = @etcd.app_op(serverId, id);
				@info("Clearing out #{op_endpoint}");
				@etcd.tryOp(-> client.del(op_endpoint, {recursive: true}));
				hold();
			} or {
				if (!alreadyRunning) {
					if (mightBeRunning) {
...
function handleOps(client, serverId, info) {
	info("watching app ops for #{serverId}");
	var prefix = @etcd.app_op(serverId)+'/';

	client
		.. @etcd.values(@etcd.app_op(serverId), {recursive:true})
		.. @filter(node -> node.value) // ignore deletions
		.. @each
	{|node|
		try {
			info("saw app op:", node.key, node.value, node);
			var suffix = node.key .. @removeLeading(prefix);
			// app ops have an autogenerated ID, so strip that off
			var [id, _opId] = suffix .. @rsplit('/',1);
...
function getLocalAppState(id) {
	var [userId, appId] = id.split('/') .. @tap(l -> l.length .. @assert.eq(2));
	var user = new @User(userId, "(slave)");
	return @app.localAppState(user, appId);
}
